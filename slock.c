		...
		struct pam_response **resp, void *appdata_ptr)
{
	...
	for (int i = 0; i < num_msg; i++) {
		if (msg[i]->msg_style == PAM_PROMPT_ECHO_OFF &&
			 ...
			 if (!resp_msg)
				  die("malloc failed\n");
			 char *password = malloc(passwd_size + 1);
			 if (!password)
				  die("malloc failed\n");
			 memset(password, 0, passwd_size + 1);
			 strncpy(password, passwd, passwd_size);
...
/* Thread that keeps track of time and refreshes it every second */
static void *
displayTime(void *input)
{
	 ...
	 while (1) {
		  ...
		  for (int i = 0; i < displayData->nscreens; ++i) {
			   refresh(
					displayData->dpy,
					displayData->locks[i]->win,
					displayData->locks[i]->screen,
					tm,
					displayData->crs[i],
...
{
	XRRScreenChangeNotifyEvent *rre = NULL;
	char buf[32];
	int num, screen;
	unsigned int color;
	KeySym ksym;
